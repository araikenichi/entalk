...
const MainApp: React.FC<{
  ...
  const handleEndLiveStream = () => {
    if (liveStreamPost) {
      setPosts(posts.map(p => p.id === liveStreamPost.id ? {
        ...p, type: PostType.Video, wasLive: true, viewers: undefined
      } : p));
    }
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case 'profile':
        const userToShow = viewingProfileId === currentUserId || !viewingProfileId
          ? userProfile
          : initialUsers[viewingProfileId] || userProfile;
...
const App: React.FC = () => {
  ...
  return (
    <I18nProvider>
      <MainApp currentUser={mockUser} onLogout={() => {}} />
    </I18nProvider>
  );
};
