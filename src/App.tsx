...
const MainApp: React.FC<{ currentUser: User; onLogout: () => void }> = ({ currentUser, onLogout }) => {
  ...
  const handleEndLiveStream = () => {
    if (liveStreamPost) {
      setPosts(
        posts.map(p =>
          p.id === liveStreamPost.id ? { ...p, type: PostType.Video, wasLive: true, viewers: undefined } : p,
        ),
      )
    }
      ...
      <div className="bg-white dark:bg-black min-h-screen text-gray-900 dark:text-gray-100">
        ...
        {showBottomNav && (
          <BottomNav
            ...
            onShowPostCreator={() => setShowGlobalPostCreator(!showGlobalPostCreator)}
          />
        )}
      </div>
    </ErrorBoundary>
...
const mockUser = (): User => ({
  ...
  name: 'Li Wei',
  handle: 'liwei88',
  avatar: asset('avatars/u2.png'), // ✅ 这里修正
    ...
    return (
      <I18nProvider>
        <div className="min-h-screen bg-white text-gray-900">
          <SignUp onSignUpSuccess={(u: User) => handleAuthSuccess(u)} onSwitchToLogin={() => setStage('login')} />
        </div>
      </I18nProvider>
    )
  }
